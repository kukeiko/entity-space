...
export class StringRangeCriterion extends RangeCriterion<string> {
    reduce(other: ValueCriterion): false | ValueCriterion<string>[] {
        if (other instanceof StringRangeCriterion) {
            ...
            if (otherFrom !== null && otherTo !== null) {
                ...
                if (fromInside && toInside) {
                    ...
                    if (selfTo === null) {
                        ...
                        return [new StringRangeCriterion([selfTo.value, otherTo.value], [selfTo.op === "<", otherTo.op === "<="])];
                    }
                } else if (toInside) {
                    if (selfFrom === null) {
                        ...
                        return [new StringRangeCriterion([otherFrom.value, selfFrom.value], [otherFrom.op === ">=", selfFrom.op === ">"])];
                    }
                    ...
                    if (fromInside && toInside) {
                        return [
                            new StringRangeCriterion([otherFrom.value, selfFrom.value], [otherFrom.op === ">=", selfFrom.op === ">"]),
                            new StringRangeCriterion([selfTo.value, otherTo.value], [selfTo.op === "<", otherTo.op === "<="]),
                        ];
                    }
                }
                ...
                if (this.isFromInsideFromTo(otherFrom)) {
                    if (selfTo === null) {
                        ...
                        return [new StringRangeCriterion([selfTo.value, void 0], selfTo.op === "<")];
                    }
                } else if (selfFrom !== null && selfTo !== null) {
                    ...
                    if (fromInside && toInside) {
                        return [
                            new StringRangeCriterion([otherFrom.value, selfFrom.value], [otherFrom.op === ">=", selfFrom.op === ">"]),
                            new StringRangeCriterion([selfTo.value, void 0], selfTo.op === "<"),
                        ];
                    }
                }
                ...
                if (this.isToInsideFromTo(otherTo)) {
                    if (selfFrom === null) {
                        ...
                        return [new StringRangeCriterion([void 0, selfFrom.value], selfFrom.op === ">")];
                    }
                } else if (selfFrom !== null && selfTo !== null) {
                    const fromInside = this.isFromInsideFromTo(selfFrom);
                    const toInside = this.isToInsideFromTo(selfTo);
                    ...
                    if (fromInside && toInside) {
                        return [
                            new StringRangeCriterion([void 0, selfFrom.value], selfFrom.op === ">"),
                            new StringRangeCriterion([selfTo.value, otherTo.value], [selfTo.op === "<", otherTo.op === "<="]),
                        ];
                    }
                }
    ...
    invert(): ValueCriterion<string>[] {
        ...
        if (this.from?.op !== void 0) {
            inverted.push(new StringRangeCriterion([void 0, this.from.value], this.from.op === ">"));
        }

        if (this.to?.op !== void 0) {
            inverted.push(new StringRangeCriterion([this.to.value, void 0], this.to.op === "<"));
        }
