...
describe("Primitive", () => {
    ...
    describe(toPrimitiveType, () => {
        it("should return Null for null", () => {
            expect(toPrimitiveType(null)).toBe(Null);
        });

        it("should return Undefined for undefined", () => {
            expect(toPrimitiveType(undefined)).toBe(Undefined);
        });

        it("should return Boolean for true", () => {
            expect(toPrimitiveType(true)).toBe(Boolean);
        });

        it("should return Boolean for false", () => {
            expect(toPrimitiveType(false)).toBe(Boolean);
        });

        it("should return Number for 1", () => {
            expect(toPrimitiveType(1)).toBe(Number);
        });

        it(`should return String for "foo"`, () => {
            expect(toPrimitiveType("foo")).toBe(String);
        });

        it("should throw error if given {}", () => {
            expect(() => toPrimitiveType({} as any)).toThrowError("value [object Object] is not a primitive value");
        });
    });
