...
describe("parse-criteria", () => {
    ...
    shouldParse("[..., 3) | (4, 7]", or([inRange(void 0, [3, false]), inRange([4, false], 7)]));
    ...
    shouldParse("{ foo: [1, 7] } | { bar: [3, 4] }", or([matches<FooBar>({ foo: inRange(1, 7) }), matches<FooBar>({ bar: inRange(3, 4) })]));

    shouldParse(
        "(([1, 7] | [3, 4]) & ({123} | !{456})) | ({1,2,3} & [-0.9, ...] & ({ foo: [1, 7] } | { bar: [3, 4] }))",
        or([
            and(or(inRange(1, 7), inRange(3, 4)), or(inSet([123]), notInSet([456]))),
            and<any>([inSet([1, 2, 3]), inRange(-0.9, void 0), or(matches<FooBar>({ foo: inRange(1, 7) }), matches<FooBar>({ bar: inRange(3, 4) }))]),
        ])
    );
});
