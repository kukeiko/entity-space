...
function getUnionDiscriminator(schemas: readonly EntitySchema[]): [string, EnumPrimitive<any>] {
...
export class EntitySchemaBuilder {
    ...
    #createEmptySchemas(): void {
        ...
        for (const [blueprint, metadata] of this.#blueprintMetadata) {
            ...
            const schema = new EntitySchema(metadata.name ?? blueprint.name, metadata.sort);
        ...
        for (const [blueprints, metadata] of this.#blueprintMetadata) {
            ...
            const schema = new EntitySchema(name, metadata.sort);
