...
describe("playground: criteria v3", () => {
    ...
    type Bar = BlueprintInstance<BarBlueprint>;
    ...
    class Builder<T> {
        takeShape<S extends EntityCriteriaShapeType<T>>(shape: S | EntityCriteriaShapeType<T>): S {
            return shape as S;
        }
    }
    ...
    interface ICriterion_<O> {
        toObject(): O;
    }

    interface ICriterionShape_<T> {
    ...
    interface ISetCriterion<S> extends ICriterion_<InSetCriterionShapeInstance<S>> {
        getValues(): (boolean | number | string | null)[];
        toObject(): InSetCriterionShapeInstance<S>;
    }

    const $ISetCriterionShape = Symbol();

    interface ISetCriterionShape<T> extends ICriterionShape_<T> {
    ...
    type PrimitiveCriteriaShapeInstance<S> = S extends Primitive | typeof Null
        ...
        : S extends ICriterionShape_<infer C>
        ? C extends ICriterion_<infer CS>
            ? CS
            : never
        : never;
    ...
    it("should work", () => {
        ...
        const fooInstance: EntityCriteriaShapeInstance<Foo, typeof fooShape> = {} as any;
        // fooInstance.id;
    ...
    it("EntityWhere<T> #2", () => {
        ...
        type PrimitiveWhere<T> =
            | (T | T[])
            | ({ $equals?: T | T[] } & (T extends number ? { $odd?: boolean; $even?: boolean } : {}) &
                  (T extends number | string ? WhereNumberOrString<T> : {}));

        // type NumOrStringWhere = PrimitiveWhere<number | string>;

        // const numOrStringWhere: NumOrStringWhere = {
        //     $equals: [1, "2"],
        // };

        type ArrayWhere<T extends unknown[]> = {
            $some?: ValueWhere<T[number]>;
            $every?: ValueWhere<T[number]>;
        } & ValueWhere<T[number]> & { $or?: ArrayWhere<T>[]; $and?: ArrayWhere<T>[] };

        type ValueWhere<T, U = Distribute<T>> = T extends ReturnType<Primitive | typeof Null>
            ? PrimitiveWhere<U> & { $or?: PrimitiveWhere<U>[]; $and?: PrimitiveWhere<U>[] }
            : EntityWhere<T> & { $or?: EntityWhere<T>[]; $and?: EntityWhere<T>[] };

        type EntityWhere<T> = {
            [K in keyof T]?: T[K] extends unknown[] | null ? ArrayWhere<Exclude<T[K], null>> : ValueWhere<T[K]>;
        };
        ...
        function whereFoo(where: EntityWhere<Foo>): void {}

        whereFoo({
            id: { $equals: [1, 3] },
            // name: { $lesser: "abc", $or: [{ $between: ["a", "z"] }, { $between: ["A", "Z"] }] },
            // primitive: [true, false, 1, "foo"],
            primitive: {
                $range: [1, 2],
                $between: ["a", "z"],
                $equals: [1, "2", null, true],
