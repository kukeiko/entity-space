...
type OptionalPropertyKeys<T> = Exclude<
    { [K in keyof T]: T[K] extends OptionalAttribute ? K : never }[keyof T],
    undefined
>;
...
type InstanceDefault<T> = {
    // [todo] document that this *has* to be "keyof T", otherwise intellisense for selections won't work.
    ...
    [K in keyof T]?: T[K] extends BlueprintProperty<infer V>
        ? NullIfNullable<T[K], BoxIfArray<T[K], PropertyValueType<Unbox<V>>>>
        : T[K];
};
