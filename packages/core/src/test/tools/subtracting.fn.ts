                ...
                shouldBe(expected: Criterion | string | boolean) {
                    if (expected === true) {
                        specFn(`${criterion} should be fully reduced by ${other}`, () => {
                            expect(parse(other).subtractFrom(parse(criterion)).toString()).toEqual("true");
                        });
                    } else if (expected === false) {
                        specFn(`${criterion} should not be reduced by ${other}`, () => {
                            expect(parse(other).subtractFrom(parse(criterion)).toString()).toEqual("false");
                        });
                    } else {
                        specFn(`${criterion} reduced by ${other} should be ${expected}`, () => {
                            expect(parse(other).subtractFrom(parse(criterion)).toString()).toEqual(
